...
const CanvasContainer = () => {
  const canvasRef = useRef(null), contextRef = useRef(null), lineSize = 3
  ...
  const updateLastClient = (x: number, y: number) => setLastClient({
    x,
    y
  })
  ...
  const draw = ({ x, y }: { x: number, y: number }) => {
    ...
    if (lastClient.x && lastClient.y && (x !== lastClient.x || y !== lastClient.y)) {
  ...
  const drawing = (event: ClientEvent) => isDrawing && draw(getClientPosition(event))
